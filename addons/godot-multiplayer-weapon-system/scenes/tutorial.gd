...
func _ready() -> void:
	...
	# Esc opens the in-game menu — but closes the open buy menu first, and is
	# suppressed (on web) while the buy menu holds the cursor.
	var pause := PauseController.new()
	pause.is_blocked = func() -> bool: return _is_buy_open()
	pause.on_escape = func() -> bool:
		if _is_buy_open():
...
func _build_signs() -> void:
	...
	var combat := "COMBAT\nFire %s · Reload %s · Throwable %s (press to cycle · LMB throw / RMB lob)\nPrimary %s · Secondary %s · Melee %s · Loadout %s" % [
		_key_for("shoot"), _key_for("reload"), _key_for("grenade"),
		_key_for("weapon_primary"), _key_for("weapon_secondary"), _key_for("weapon_melee"), _key_for("buy")]
